...
=== Exam-4-1-1. MovableBall class를 구현하라.[[Exam-4-1-1]]
...
=== Exam-4-1-2. MovableBall class를 JUnit을 이용해 확인하라.[[Exam-4-1-2]]
...
*** 이동에 문제가 없나?
*** <<Exam-4-1-1>>과 어떠한 차이가 있나?
*** 동일한 테스트 과정을 여러 방법으로 구현해 보았다. 차이와 장단점은?

== 4-2. MovableWorld class

=== 정의

* <<Exam-4-1-1>>과 <<Exam-4-1-2>>에서는 world에 등록했지만, main에서 ball을 이동 시킴
* world에서 ball을 이동 시킴
...
=== Exam-4-2-1. MovableWorld class를 구현하라[[exam-3-2-1]]

* MovableWorld
** World class 확장
** World class의 변수를 직접 접근해야 할 경우, getter 추가 후 이용
* ball
** 10개 생성
** 시작 위치 window 내로 제한
** 크기는 반지름 10~50 사이
...
=== Exam-4-3-1. 다음 코드는 MovableBall을 수정하여 로그를 추가하여 ball 이동이 보이지 않는 문제에 관해 확인해 보자
...
[source,console]
----
2024-01-28T00:07:41.786+0900 TRACE ball_05 move to : x = 433, y = 102
...
2024-01-28T00:07:41.787+0900 TRACE ball_01 move to : x = 412, y = 401
...
2024-01-28T00:07:41.788+0900 TRACE ball_05 move to : x = 454, y = 120
...
=== Exam-4-3-2. MovableWorld class에 단위 시간을 추가하라
...
=== Exam-4-3-3. MovableWorld에서 move 후 변경되는 시간의 오차를 최소화하라
