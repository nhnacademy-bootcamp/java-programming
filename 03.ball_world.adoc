...
=== Exam-3-1-2. Ball class에 볼이 차지하는 영역 확인해 보자
...
=== Exam-3-1-3. 볼 생성시 주어지는 값들이 볼 생성 조건을 만족하는지 확인해 보자
...
=== Exam-3-1-4. JUnit을 이용해 테스트 코드를 작성하여 확인해 보라.

* Exam-3-1-3에서의 확인 과정을 https://junit.org/junit5/docs/current/user-guide[JUnit5]를 이용해 테스트 코드로 작성하라.
...
=== Exam-3-1-5. JUnit을 이용해 랜덤 테스트를 작성하고 확인해 보자.
...
=== Exam-3-2-1. PaintableBall class를 정의하고, 화면에 출력되는지 확인하라. [[exam-2-2-1]]
...
=== Exam-3-2-2. PaintableBall class를 JUnit test code를 이용해 확인하라.
...
=== Exam-3-2-3. Color를 지정하지 않을 경우, 기본 값을 사용하도록 수정하라.
...
=== Exam-3-3-1. World class를 구현하라[[exam-2-3-1]]
...
=== Exam-3-3-2. 구현된 world에 paintable ball을 추가해 출력하라
...
=== Exam-3-3-3. 추가되는 ball이 World의 영역을 벗어날 경우 exception을 발생시키고 새롭게 생성하라.
